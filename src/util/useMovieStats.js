...
/**
 * Derives totals, averages, highest/lowest, and profit from an array of
 * TMDB movie detail objects. Memoised so it only recalculates when the list changes.
 */
export function useMovieStats(list) {
    return useMemo(() => {
        ...
        const highestBudget = list.reduce((max, m) => Math.max(max, m.budget || 0), 0);
        const highestRevenue = list.reduce((max, m) => Math.max(max, m.revenue || 0), 0);
        const highestRuntime = list.reduce((max, m) => Math.max(max, m.runtime || 0), 0);
        const highestRating = list.reduce((max, m) => Math.max(max, m.vote_average || 0), 0);
